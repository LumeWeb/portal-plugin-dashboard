...
func TestSolanaHandler_VerifyProof_ExpiredNonce(t *stdtesting.T) {
	...
	challengeSvc := caip122.NewChallengeService(h.getStore(), caip122.DefaultChallengeConfig(domain))

	// Set a nonce with TTL=0 so it expires immediately.
	const expiredNonce = "expirednonce1"
	err := h.getStore().Set(nil, expiredNonce, domain, 0)
	require.NoError(t, err)
...
func TestSolanaHandler_SetStore(t *stdtesting.T) {
	h := NewSolanaHandler()
	originalStore := h.getStore()

	// Set nil store — should be a no-op
	h.SetStore(nil)
	assert.Equal(t, originalStore, h.getStore())

	// Set a new store
	newStore := caip122.NewMemoryChallengeStore()
	h.SetStore(newStore)
	assert.Equal(t, newStore, h.getStore())
	defer newStore.Close()
}

func TestSolanaHandlerRegistration(t *stdtesting.T) {
...
func TestSolanaHandler_VerifyProof_InvalidKey(t *stdtesting.T) {
	...
	// Store the nonce
	_ = h.getStore().Set(nil, "somenonce123", "test.example.com", 0)
