...
func TestEthereumHandlerRegistration(t *stdtesting.T) {
	reg := EthereumHandlerRegistration()
	if reg.Type != "ethereum" {
		t.Fatalf("expected type 'ethereum', got %q", reg.Type)
	}
	if reg.Handler == nil {
...
func TestSetStore_ClosesPreviousStore(t *stdtesting.T) {
	h := NewEthereumHandler()
	oldStore := h.getStore().(*caip122.MemoryChallengeStore)
...
func TestMemoryChallengeStore_SetCap(t *stdtesting.T) {
	...
	// The next Set should exceed capacity and return an error.
	err := store.Set(ctx, "overflow-nonce", "localhost", 5*time.Minute)
	if err == nil {
		t.Fatal("expected error when exceeding store capacity")
	}
